...
def _vivino_ours(
    config: orders_client.DbConfig, merchant: str
) -> pd.DataFrame:
    """Read one merchant's own catalogue for either Vivino comparison path."""
...
@read_log.logged_read("app._crawled_comparison_cached")
@st.cache_data(ttl=_CRAWLER_TTL_SECONDS, show_spinner=False)
def _crawled_comparison_cached(
    source: str, merchant: str, slug: str
) -> vivino_client.Comparison | None:
    """Compare the fast daily crawl, or return none when its merchant is absent."""
    read_log.mark_executed()
    config = orders_client.load_medusa_env()
    if config is None or config.label != source:
        raise orders_client.MedusaConfigError(
    ...
    ours = _vivino_ours(config, merchant)
...
def _render_vivino(chosen: str, picker: bool = True) -> None:
    ...
    if not slug:
        ...
        return
    # The daily crawler is quick enough to try on every rerun. Only the live
    # page-by-page read is held in session state, because it can take minutes
    # and should not restart when the shared cache is refreshed elsewhere.
    reads = st.session_state.setdefault("vivino_reads", {})
    crawler_unavailable = False
    try:
        result = _crawled_comparison_cached(config.label, chosen, slug)
    except Exception as exc:  # noqa: BLE001 - the live fallback stays available
        ...
        if held and time.time() - held[0] < _VIVINO_TTL_SECONDS:
            ...
            try:
                with st.spinner(f"Reading {chosen}'s Vivino shop, page by page..."):
                    live = _vivino_comparison_cached(config.label, chosen, slug)
